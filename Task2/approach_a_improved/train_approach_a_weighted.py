"""
Task 2 — Approach A Improved: ResNet-18 with Weighted CrossEntropy Loss

Identical to train_approach_a.py but adds class weighting to address
histiocyte underperformance — the rarest and hardest class in the test set.

Change: CrossEntropyLoss(weight=[2.0, 1.0, 1.0])
  - histiocyte (idx 0) weighted 2x — misclassifying it costs double
  - lymphocyte / tumor remain at 1.0
"""
...
OUTPUT_DIR = Path(__file__).parent / "checkpoints_a_weighted"
...
# Histiocyte weight = 2.0 — penalise misclassification of rarest/hardest class
...
def main():
    ...
    ax1.set_title("Loss (Weighted)"); ax1.set_xlabel("Epoch"); ax1.legend()
    ...
    ax2.set_title("Accuracy (Weighted)"); ax2.set_xlabel("Epoch"); ax2.legend()
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "training_curves_a_weighted.png", dpi=150)
    plt.close()
